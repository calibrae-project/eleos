...
function generateMemoTable(memos) {
  ...
  for (let i = 0; i < localMemos.length; i++) {
    localMemos[i]['details'] =
      '<a href="javascript:void(0)" onclick="renderer.showTxDetails(\'' +
      localMemos[i].txid +
      '\')">click</a>';
...
function generateHistoryTable(txs, privTxs) {
  ...
  for (let i = 0; i < privTxs.length; i++) {
    privTxs[i].address =
      privTxs[i].address.substr(0, 16) +
      '......' +
      privTxs[i].address.substr(-16);
  }
  memos = [];
  ...
  let div = document.createElement('div');
  div.innerHTML = tableElement;
  div.firstElementChild.className += 'w3-table-all w3-medium';
  for (let i = 0; i < div.getElementsByClassName('number').length; i++) {
    div.getElementsByClassName('number')[i].className += ' w3-right';
  }
  document.getElementById('transactionTransparentSpan').innerHTML =
    div.innerHTML;
}
...
ipcRenderer.on('jsonQuery-reply', (event, arg) => {
  if (arg.error && arg.error.code === -28) {
    document.getElementById('alertSpan').innerHTML =
      '<img src="resources/box.gif" style="display: block; margin: auto; padding-top: 15px;"/><h2 style="text-align: center;">' +
      arg.error.message +
      '</h2>';
  } else {
    document.getElementById('alertSpan').innerHTML = '';
  ...
  if (arg.id === 'getnetworkinfo' && arg.result) {
    document.getElementById('connectionsValue').innerHTML =
      arg.result.connections;
    ...
    document.getElementById('transparentBalanceValue').innerHTML =
      arg.result.transparent;
    document.getElementById('transparentAvailableValue').innerHTML =
      arg.result.transparent;
    document.getElementById('privateBalanceValue').innerHTML =
      arg.result.private;
    ...
    for (let i = 0; i < arg.result.length; i++) {
      for (let n = 0; n < arg.result[i].length; n++) {
        table[ctr] = {
          'transparent address': arg.result[i][n][0],
          amount: arg.result[i][n][1]
        };
        ctr += 1;
    ...
    if (arg.result.length < 1) {
      table[0] = {
        'transparent address': 'No addresses with received balances found',
        amount: 0
      };
    }
    ...
    if (
      document.getElementById('addressTransparentSpan').innerHTML !==
      div.innerHTML
    ) {
      document.getElementById('addressTransparentSpan').innerHTML =
        div.innerHTML;
    ...
    if (arg.result.length < 1) {
      table[0] = {
        privateaddress: 'No addresses with received balances found',
        amount: 0
      };
    }
    let tableElement = tableify(table);
    ...
    for (let i = 0; i < arg.result.length; i++) {
      let res = generateQuerySync('z_listreceivedbyaddress', [
        arg.result[i],
        0
      ]);
    ...
    if (arg.result === null) {
      ...
      window.alert(
        'Successfully transmitted transaction.\n\nTXID: ' + arg.result
      );
    }
    ...
    if (arg.result === null) {
      ...
      window.alert(
        'Successfully initiated private transaction.\n\nTXID: ' + arg.result
      );
    }
  }
});
...
ipcRenderer.on('params-pending', (event, arg) => {
  if (arg.percent < 1) {
    document.getElementById('alertSpan').innerHTML =
      '<img src="resources/box.gif" style="display: block; margin: auto; padding-top: 15px;"/>' +
      '<h2 style="text-align: center;">downloading proving and verification keys</h2>' +
      '<h4 style="text-align: center;">' +
      arg.name.substr(arg.name.lastIndexOf('/') + 1) +
      '</h4>' +
      '<h4 style="text-align: center;">' +
...
ipcRenderer.on('params-complete', (event, arg) => {
  if (arg === false) {
    document.getElementById('alertSpan').innerHTML =
      ...
      '<img src="resources/box.gif" style="display: block; margin: auto; padding-top: 15px;"/>' +
      '<h2 style="text-align: center;">initializing</h2></div>';
  } else {
    document.getElementById('alertSpan').innerHTML = '';
  }
});
...
function refreshUI() {
  ...
  // for historyPage
  generateQuery('listtransactions', ['*', 5000]);
